...
const TIFrameworkHero = () => {
  ...
  return (
    ...
    >
      {/* Video Background */}
      <div className="absolute inset-0 bg-black/100 overflow-hidden">
        <video
          autoPlay
          muted
          loop
      ...
      <div className="relative z-10 w-full px-[18px] sm:px-[32px] md:px-[56px] lg:px-[56px] pt-0 pb-[124px] max-[1100px]:pb-[96px]">
        <h1
          className={`m-0 font-serif font-bold leading-[1.05] -tracking-[0.045em] text-white will-change-[transform,opacity]
            ...
            text-[clamp(1.75rem,4.5vw,3.125rem)] max-[1100px]:text-[clamp(1.75rem,5.5vw,2.8rem)] max-[640px]:text-[2.2rem]
          `}
        >
          The TI (Technology
          <br />
        ...
        <p
          className={`mt-[42px] max-w-[980px] font-serif font-light leading-[1.4] text-[#cfd4e6] will-change-[transform,opacity]
            opacity-0 translate-y-[72px]
            ${isInView ? "animate-ti-desc" : ""}
            text-[clamp(1rem,1.5vw,1.5rem)] max-[1100px]:mt-7 max-[1100px]:text-[clamp(1.1rem,2.4vw,1.6rem)] max-[640px]:mt-5 max-[640px]:text-lg
          `}
        >
          ...
          process framework &amp; guidance <br /> for Digital Strategy.
        </p>
      </div>
    </section>
  );
